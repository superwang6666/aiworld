...
export function synthesizeWithWeights(
  ...
  for (const lawWeight of lawWeights) {
    ...
    for (const expertResponse of expertResponses) {
      ...
      const expertConfig = expertConfigs?.find(ec => ec.id === expertResponse.expert_id);
    ...
    // 归一化得分（如果有专家贡献）
    const finalScore = totalExpertiseDepth > 0
      ? totalWeightedScore / totalExpertiseDepth
      : 0;
    ...
    predictions.push({
      ...
      contributingExperts: expertContributions.sort((a, b) => b.weight - a.weight), // 按权重降序
    });
  }
 ...
 * 识别专家共识
 */
export function identifyConsensus(
  ...
  // 高优先级法则（加权分数前3）
  const highPriorityLaws = predictions
    .slice(0, 3)
    .map(p => p.law);
  ...
  const convergentPredictions = predictions.map(p => ({
    law: p.law,
    consensusLevel: p.contributingExperts.length / predictions[0].contributingExperts.length,
  }));
...
export function detectDisagreements(
  ...
  // 按法则分组专家预测
  const predictionsByLaw = new Map<Law, {
    expert: string;
    prediction: string;
    confidence: number;
  }[]>();
...
export function generateEmergentInsights(
  ...
  const highConsensus = consensusAnalysis.convergentPredictions.filter(c => c.consensusLevel > 0.8);
  ...
  // 洞察3: 次级效应
  const secondaryEffects = predictions.filter(p =>
    p.weightedScore > 0.1 && !highImpactLaws.includes(p.law)
  );
