 ...
 * @returns 法则权重数组
 */
export function calculateLawWeights(
  ...
  for (const law of ALL_LAWS) {
    ...
    const mentionsInCore = (
      coreAnomalyText.match(new RegExp(lawLower, "g")) || []
    ).length;
    const mentionsInEraser = (
      eraserAnalysis.match(new RegExp(lawLower, "g")) || []
    ).length;
  ...
  // 步骤4: 归一化权重（总和=1）
  const totalRawScore = Array.from(rawScores.values()).reduce(
    (sum, score) => sum + score,
    0,
  );
  ...
  for (const [law, score] of rawScores.entries()) {
    normalizedWeights.set(
      law,
      totalRawScore > 0 ? score / totalRawScore : 1 / ALL_LAWS.length,
    );
  }
...
export function validateRuleDistribution(
  ...
  for (const lw of lawWeights) {
    ...
    if (diff > tolerance) {
      errors.push(
        `${lw.law}: 期望${expected}条规则，实际${actual}条（误差${diff}）`,
      );
    }
  }

  // 检查总数
  const totalActual = Array.from(actualCounts.values()).reduce(
    (sum, count) => sum + count,
    0,
  );
