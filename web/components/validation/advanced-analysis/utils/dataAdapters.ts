 ...
 * @returns 专家卡片数据
 */
export function adaptExpertResponse(response: ExpertResponse): ExpertCardData {
  return {
    ...
    warning: response.warnings?.[0] || "暂无警告",
    suggestion: response.suggestions?.[0] || "暂无建议"
  };
}
 ...
 * @returns 适配后的数据或null
 */
export function adaptDEACAnalysis(analysis: DEACAnalysis | null) {
  if (!analysis) return null;

  return {
    experts: analysis.expert_responses.map(adaptExpertResponse),
    synthesis: {
